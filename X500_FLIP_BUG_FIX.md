...
## Phase 6: Tích hợp Robotic Arm vào X500

### Mục tiêu
Thêm cánh tay robot 2-DOF (shoulder + elbow) vào drone x500, điều khiển qua
ArduPilot servo (FUNCTION=0 + MAV_CMD_DO_SET_SERVO).

### Kiến trúc

```
x500_base/model.sdf             ← Frame, links, sensors
arm/model.sdf                   ← 2-DOF arm (links, joints, meshes)
    ↓            ↓
x500_with_arm/model.sdf         ← Wrapper: include x500_base + arm
                                   + fixed joint + LiftDrag + ArduPilotPlugin
                                   + JointPositionController
```

### Arm Model (arm/model.sdf)

| Link | Mass (kg) | Mô tả |
|------|-----------|-------|
| arm_base_link | 0.049 | Đế gắn vào drone |
| arm_shoulder_link | 0.081 | Link vai |
| arm_elbow_link | 0.092 | Link khuỷu |

| Joint | Type | Axis | Range | Damping | Friction |
|-------|------|------|-------|---------|----------|
| shoulder_joint | revolute | Y | 0 → 3.14 rad (0°–180°) | 0.1 | 0.05 |
| elbow_joint | revolute | Y | -1.57 → 1.57 rad (±90°) | 0.1 | 0.05 |

Mesh URIs dùng `package://ardupilot_gazebo/models/arm/` cho RViz compatibility.

### Servo Control Flow

```
MAV_CMD_DO_SET_SERVO (cmd 183)
  → ArduPilot SITL (SERVO9/10, FUNCTION=0)
    → ArduPilotPlugin (type=COMMAND, normalize PWM)
      → Gazebo topic (/arm/cmd_shoulder, /arm/cmd_elbow)
        → JointPositionController (PID → joint force)
```

### ArduPilot Channel Mapping

| Plugin channel | ArduPilot | Param | PWM Range | Góc quay |
|----------------|-----------|-------|-----------|----------|
| ch8 | SERVO9 (AUX1) | FUNCTION=0 | 1100–1900 | 0°–180° (shoulder) |
| ch9 | SERVO10 (AUX2) | FUNCTION=0 | 1100–1900 | -90°–+90° (elbow) |

### JointPositionController PID Tuning

| Param | Giá trị | Vai trò |
|-------|---------|---------|
| p_gain | 2 | Lực tỉ lệ với error |
| i_gain | 0.3 | Tích lũy error → vượt friction khi gần target |
| d_gain | 0.3 | Giảm chấn, bớt overshoot |
| cmd_max | 0.3 | Giới hạn torque → servo quay chậm, không rung drone |

### Arm Control Node (arm_mission.py)

Chương trình ROS2 điều khiển drone + arm:

```
GUIDED → ARM → Takeoff → Hover → Arm Sequence → Hover → LAND
```

Arm sequence gồm 5 bước, mỗi bước gửi PWM cho cả 2 servo và giữ 3 giây.
Sử dụng `MAV_CMD_DO_SET_SERVO` (command 183) qua MAVROS service `/mavros/cmd/command`.

### Files mới/sửa

| File | Thay đổi |
|------|----------|
| `src/ardupilot_gazebo/models/arm/model.sdf` | Fix mass (0.001→thật), damping 0.01→0.1, friction 0.05, URIs `package://`, bỏ JointPositionController |
| `src/ardupilot_gazebo/models/x500_with_arm/model.sdf` | **Mới**: wrapper include x500_base + arm, LiftDrag, ArduPilotPlugin (ch0-3 motors + ch8-9 servo COMMAND), JointPositionController PID |
| `src/ardupilot_gazebo/models/x500_with_arm/model.config` | **Mới**: model metadata |
| `src/ardupilot_gazebo/config/gazebo-x500.parm` | Thêm SERVO9/10 FUNCTION=0, MIN/MAX/TRIM |
| `src/ardupilot_gz/.../robots/x500_with_arm.launch.py` | **Mới**: robot launch config |
| `src/ardupilot_gz/.../x500_with_arm_runway.launch.py` | **Mới**: full launch (Gazebo + SITL + RViz) |
| `src/drone_control/.../arm_mission.py` | **Mới**: takeoff + arm control + land mission node |
| `src/drone_control/setup.py` | Thêm entry_point `arm_mission` |

### Điều khiển manual

```bash
# Shoulder 90° (PWM 1500)
ros2 service call /mavros/cmd/command mavros_msgs/srv/CommandLong \
  "{command: 183, param1: 9, param2: 1500}"

# Elbow +45° (PWM 1700)
ros2 service call /mavros/cmd/command mavros_msgs/srv/CommandLong \
  "{command: 183, param1: 10, param2: 1700}"
```
